...
class Board:
    ...
    def reset(self):
        reset = Button(root, text='Reset', font=UIFONT, background=RED, command=lambda:(self.destroy(),self.__init__(self.single)), activebackground=RED)
        reset.grid(row=3,column=2)
        menu = Button(root, text='Menu', font=UIFONT, background=GREEN, command=lambda:(self.destroy(),create_ui()),activebackground=GREEN)
...
def create_level():
    ...
    level = Scale(root, from_=1, to=10, orient=HORIZONTAL, background=GREEN, troughcolor=RED, font=UIFONT, sliderlength=25, length=200, relief=RAISED, sliderrelief=FLAT, activebackground=BLUE)
...
def create_ui():
    ...
    playbutton = Button(text='Play', font=UIFONT, background=GREEN, height=1, width=7, command=play, activebackground=GREEN)
    playbutton.grid(row=1,columnspan=6)
    gamemode = Label(textvariable=mode, font=UIFONT, background=GREEN, width=10)
    gamemode.grid(row=2,column=1)
    leftarrow = Button(width=3, text='<', font=BUTTONFONT, command=modechange, background=BLUE, activebackground=BLUE)
    rightarrow = Button(width=3, text='>', font=BUTTONFONT, command=modechange, background=BLUE, activebackground=BLUE)
